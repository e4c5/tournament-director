...
from pairing.models import Standing, Player
            ...
            if current_player != player :
                if 'pair' not in player or player['pair'] is False:
                    if player['name'] not in current_player['opponents'] : 
                        if current_player['name'] == 'Zeena Lye':
                            print('yep appending for ({0})'.format(current_player['name']))
                            print('the opponents', player['opponents'])
                        rest.append(player)
...
class DbPairing(Pairing):
    ...
    def __init__(self, next_round=1):
        ...
        self.bye = Player.objects.get(name='bye')
        
        qs = Standing.objects.select_related().filter(tournament_id=self.tournament.id)
        self.players = []

        for pl in Player.objects.select_related():
            opponents = []
            spread = 0
            wins = 0
            for game in pl.player1_set():
                spread += game.score1 - game.score2
                opponents.append(game.player2.name)
                
                if game.score1 > game.score2 : wins += 1
                elif game.score1 == game.score2: wins += 0.5
                    
            for game in pl.player2_set():
                spread += game.score2 - game.score1
                opponents.append(game.player1.name)
                
                if game.score2 > game.score1 : wins += 1
                elif game.score1 == game.score2: wins += 0.5
